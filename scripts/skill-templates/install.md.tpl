...
Design Vibes 是一个网页设计风格库：{{COUNT}} 种风格，每种带完整的中英提示词、精确的 Design Tokens、避坑清单和一个零依赖单文件 HTML 参考实现。装成 skill 之后，直接说「做一个赛博朋克风格的落地页」就能用。

来源：{{HOMEPAGE}} · {{REPO}} · MIT
...
mkdir -p "$DIR" && curl -fsSL {{HOMEPAGE}}/{{TARBALL}} | tar -xz -C "$DIR"
```
...
```bash
DIR=~/.claude/skills/design-vibes
mkdir -p "$DIR" && curl -fsSL {{REPO}}/archive/refs/heads/main.tar.gz | tar -xz --strip-components=3 -C "$DIR" design-vibes-main/skills/design-vibes
```

**第 3 步 · 验证。** 确认这三项都成立，任何一项不成立就说明没装好：

- `$DIR/SKILL.md` 存在
- `$DIR/references/styles/` 下有 {{COUNT}} 个 `.md` 文件
- `$DIR/assets/demos/` 下有 {{COUNT}} 个 `.html` 文件
